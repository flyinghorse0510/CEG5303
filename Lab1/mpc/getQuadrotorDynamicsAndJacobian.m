...
% b: drag constant
% g: gravity
% u(1:4): squared angular velocity of rotor i as control input
% u(5:7): wind velocity with respect to earth frame
syms k l m b g u1 u2 u3 u4 u5 u6 u7
...
syms rho_air drag_coefficient area_x area_y area_z

area = [area_x; area_y; area_z];

relative_v_w = R' * (v_w - [diff(x,t); diff(y,t); diff(z,t)]);
...
f = [ % Set time-derivative of the positions and angles
      ...
      -g*[0;0;1] + R*[0;0;T]/m + R*0.5*rho_air*drag_coefficient* (area .* (relative_v_w.^2)) / m;
...
bVal = 0.2;
gVal = 9.81;
rho_airVal = 1.225; % air density in [kg/m^3]
drag_coefficientVal = 0.9;
area_xVal = 0.03; % wind exposed area in m^2
area_yVal = 0.03;
area_zVal = 0.0848;

f = subs(f, [Ixx Iyy Izz k l m b g rho_air drag_coefficient area_x area_y area_z], ...
    [IxxVal IyyVal IzzVal kVal lVal mVal bVal gVal rho_airVal drag_coefficientVal area_xVal area_yVal area_zVal]);
f = simplify(f);

% Calculate Jacobians for nonlinear prediction model
A = jacobian(f,state);
control = [u1; u2; u3; u4; u5; u6; u7];

function map = load_map_larger(filename, xy_res, z_res, margin)
%  MAP = LOAD_MAP(filename, xy_res, z_res, margin).  
% Creates an occupancy grid map where a node is considered fill if it lies within 'margin' distance of on abstacle.
fid = fopen(filename);
tline = fgets(fid);
blocks = [];
...
blocks(:,1:2) = blocks(:,1:2)-0.25;
blocks(:,3) = blocks(:,3)-0.15;
blocks(:,4:5) = blocks(:,4:5)+0.25;
blocks(:,6) = blocks(:,6)+0.15;
map.blocks = blocks;
map.xy_res = xy_res;
map.z_res = z_res;
...
for i = 1:size(blocks,1)
    ...
    occ_map(xs:xe,ys:ye,zs:ze) = 1;%Set the corresponding grid area to occupied
end
map.map3d = map3d;

map3d_col = ones(n_x, n_y, n_z, 3)*255;
for i = 1:size(blocks,1)
    block = blocks(i, :);
    ...
    xe = xyze(1); ye = xyze(2); ze = xyze(3);
    % convert block to coordinates
    map3d_col(xs:xe,ys:ye,zs:ze,1) = uint8(block(7));
    map3d_col(xs:xe,ys:ye,zs:ze,2) = uint8(block(8));
    map3d_col(xs:xe,ys:ye,zs:ze,3) = uint8(block(9));
end
map.map3d_collision = map3d_col;
